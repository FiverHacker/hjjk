...
def setup_container(container_id, memory, vps_id, ssh_port, root_password, watermark, welcome, disk):
    try:
        ...
        # Set memory limit via cgroups to fix memory display
        memory_mb = memory * 1024
        memory_bytes = memory * 1024 * 1024 * 1024
        mem_limit_cmd = f"""
        # Set memory limit via cgroup v1
        if [ -d /sys/fs/cgroup/memory ]; then
            echo {memory_bytes} > /sys/fs/cgroup/memory/memory.limit_in_bytes 2>/dev/null || true
            echo {memory_bytes} > /sys/fs/cgroup/memory/memory.memsw.limit_in_bytes 2>/dev/null || true
        fi
        # Set memory limit via cgroup v2
        if [ -d /sys/fs/cgroup ]; then
            echo {memory_bytes} > /sys/fs/cgroup/memory.max 2>/dev/null || true
            echo {memory_bytes} > /sys/fs/cgroup/memory.swap.max 2>/dev/null || true
        fi
        """
        run_docker_command(container_id, ["bash", "-c", mem_limit_cmd])
        
        # Allocate disk space using loopback device to extend root filesystem
        disk_gb = disk
        disk_file = f"/disk-{vps_id}.img"
        
        # Create disk allocation script that extends root filesystem
        # This creates a loopback device and uses it to provide the allocated disk space
        disk_setup_cmd = f"""
        DISK_FILE={disk_file}
        DISK_SIZE={disk_gb}G
        
        # Create sparse disk image file (sparse so it doesn't take actual space until used)
        if [ ! -f $DISK_FILE ]; then
            fallocate -l $DISK_SIZE $DISK_FILE 2>/dev/null || \
            dd if=/dev/zero of=$DISK_FILE bs=1G count={disk_gb} seek=0 2>/dev/null || \
            truncate -s $DISK_SIZE $DISK_FILE 2>/dev/null
        fi
        
        # Find and setup loopback device
        LOOP_DEV=$(losetup -f 2>/dev/null)
        if [ -n "$LOOP_DEV" ] && [ -f $DISK_FILE ]; then
            # Check if already attached
            if ! losetup -a | grep -q "$DISK_FILE"; then
                # Attach loopback device
                losetup $LOOP_DEV $DISK_FILE 2>/dev/null || true
            else
                # Get existing loop device
                LOOP_DEV=$(losetup -a | grep "$DISK_FILE" | cut -d: -f1)
            fi
            ...
            if [ -n "$LOOP_DEV" ]; then
                ...
                mkdir -p /mnt/vps-disk
                mount $LOOP_DEV /mnt/vps-disk 2>/dev/null || true
                
                # Create bind mounts to make disk space accessible from common locations
                mkdir -p /root/vps-storage /home/vps-storage /var/vps-storage /opt/vps-storage
                mount --bind /mnt/vps-disk /root/vps-storage 2>/dev/null || true
                mount --bind /mnt/vps-disk /home/vps-storage 2>/dev/null || true
                mount --bind /mnt/vps-disk /var/vps-storage 2>/dev/null || true
                mount --bind /mnt/vps-disk /opt/vps-storage 2>/dev/null || true
                
                # Add to fstab for persistence
                if ! grep -q "/mnt/vps-disk" /etc/fstab 2>/dev/null; then
                    echo "$LOOP_DEV /mnt/vps-disk ext4 defaults 0 2" >> /etc/fstab
                    echo "/mnt/vps-disk /root/vps-storage none bind 0 0" >> /etc/fstab
                    echo "/mnt/vps-disk /home/vps-storage none bind 0 0" >> /etc/fstab
                    echo "/mnt/vps-disk /var/vps-storage none bind 0 0" >> /etc/fstab
                    echo "/mnt/vps-disk /opt/vps-storage none bind 0 0" >> /etc/fstab
                fi
                
                # Create startup script to mount disk on boot
                cat > /usr/local/bin/mount-vps-disk.sh << 'EOFSCRIPT'
#!/bin/bash
DISK_FILE={disk_file}
LOOP_DEV=$(losetup -f 2>/dev/null)
if [ -n "$LOOP_DEV" ] && [ -f $DISK_FILE ]; then
    ...
    if [ -n "$LOOP_DEV" ]; then
        mkdir -p /mnt/vps-disk /root/vps-storage /home/vps-storage /var/vps-storage /opt/vps-storage
        mount $LOOP_DEV /mnt/vps-disk 2>/dev/null || true
        mount --bind /mnt/vps-disk /root/vps-storage 2>/dev/null || true
        mount --bind /mnt/vps-disk /home/vps-storage 2>/dev/null || true
        mount --bind /mnt/vps-disk /var/vps-storage 2>/dev/null || true
...
EOFSCRIPT
                # Replace placeholder
                sed -i "s|{disk_file}|$DISK_FILE|g" /usr/local/bin/mount-vps-disk.sh
                chmod +x /usr/local/bin/mount-vps-disk.sh
                
                # Add to rc.local or create systemd service for boot persistence
                if [ -f /etc/rc.local ]; then
                    if ! grep -q "mount-vps-disk" /etc/rc.local; then
                        sed -i '/^exit 0/i /usr/local/bin/mount-vps-disk.sh' /etc/rc.local
                        chmod +x /etc/rc.local
                    fi
                else
                    # Create rc.local if it doesn't exist
                    echo '#!/bin/bash' > /etc/rc.local
                    echo '/usr/local/bin/mount-vps-disk.sh' >> /etc/rc.local
                    echo 'exit 0' >> /etc/rc.local
                    chmod +x /etc/rc.local
                fi
                
                # Also create systemd service for better integration
                cat > /etc/systemd/system/vps-disk.service << 'EOFSERVICE'
[Unit]
Description=VPS Disk Mount Service
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/local/bin/mount-vps-disk.sh

[Install]
WantedBy=multi-user.target
EOFSERVICE
                systemctl daemon-reload 2>/dev/null || true
                systemctl enable vps-disk.service 2>/dev/null || true
                systemctl start vps-disk.service 2>/dev/null || true
            fi
        fi
        """
        success, output, stderr = run_docker_command(container_id, ["bash", "-c", disk_setup_cmd])
        
        # Create the show resources script separately (after disk is set up)
        create_show_script = f"""
        echo '#!/bin/bash' > /usr/local/bin/show-vps-resources.sh
        echo 'echo "=== VPS Allocated Resources ==="' >> /usr/local/bin/show-vps-resources.sh
        echo 'echo "Memory Limit: {memory}GB"' >> /usr/local/bin/show-vps-resources.sh
        echo 'echo "Disk Space: {disk_gb}GB"' >> /usr/local/bin/show-vps-resources.sh
        echo 'echo ""' >> /usr/local/bin/show-vps-resources.sh
        echo 'echo "=== Disk Usage ==="' >> /usr/local/bin/show-vps-resources.sh
        echo 'df -h /mnt/vps-disk 2>/dev/null || echo "Disk not mounted. Run: /usr/local/bin/mount-vps-disk.sh"' >> /usr/local/bin/show-vps-resources.sh
        echo 'echo ""' >> /usr/local/bin/show-vps-resources.sh
        echo 'echo "=== Memory Usage ==="' >> /usr/local/bin/show-vps-resources.sh
        echo 'free -h' >> /usr/local/bin/show-vps-resources.sh
        echo 'echo ""' >> /usr/local/bin/show-vps-resources.sh
        echo 'echo "=== All Mounted Disks ==="' >> /usr/local/bin/show-vps-resources.sh
        echo 'df -h | grep -E "(Filesystem|/mnt/vps-disk|/root/vps-storage)"' >> /usr/local/bin/show-vps-resources.sh
        chmod +x /usr/local/bin/show-vps-resources.sh
        """
        run_docker_command(container_id, ["bash", "-c", create_show_script])
        if success:
            logger.info(f"Disk {disk_gb}GB and memory {memory}GB limits configured for VPS {vps_id}")
        else:
            logger.warning(f"Resource allocation completed with warnings for VPS {vps_id}: {stderr}")
        ...
        # Start SSH service - critical for SSH access
        # Docker containers may not have systemd, so we need multiple fallback methods
        ensure_ssh_cmd = """
        # Create necessary directories
        mkdir -p /var/run/sshd
        chmod 755 /var/run/sshd
        
        # Ensure SSH config allows root login and password auth
        sed -i 's/#PermitRootLogin prohibit-password/PermitRootLogin yes/' /etc/ssh/sshd_config 2>/dev/null || true
        sed -i 's/PermitRootLogin prohibit-password/PermitRootLogin yes/' /etc/ssh/sshd_config 2>/dev/null || true
        ...
        if pgrep -x sshd > /dev/null || pgrep -f '/usr/sbin/sshd' > /dev/null; then
            echo "SSH is already running"
        else
            # Try to start SSH using systemd (if available)
            if systemctl is-active --quiet ssh 2>/dev/null || systemctl is-active --quiet sshd 2>/dev/null; then
                echo "SSH is active via systemd"
            else
                # Try systemd start
                systemctl start ssh 2>/dev/null || \
                systemctl start sshd 2>/dev/null || \
                # Fallback: start SSH daemon directly in background
                /usr/sbin/sshd -D &
                sleep 3
            fi
        fi
        ...
        if netstat -tlnp 2>/dev/null | grep -q ':22 ' || ss -tlnp 2>/dev/null | grep -q ':22 '; then
            ...
            # Try one more time to start
            /usr/sbin/sshd -D &
            sleep 2
        fi
        
        # Final check
        pgrep -x sshd > /dev/null && echo "SSH daemon process found" || echo "WARNING: SSH daemon process not found"
        """
        success, output, stderr = run_docker_command(container_id, ["bash", "-c", ensure_ssh_cmd])
        if success:
            logger.info(f"SSH service configured for VPS {vps_id}. Output: {output[:200]}")
        else:
            logger.warning(f"SSH startup completed with warnings for VPS {vps_id}: {stderr}")
        
        # Create a startup script to ensure SSH starts on container boot
        ssh_startup_script = """
        cat > /usr/local/bin/start-ssh.sh << 'EOFSSH'
#!/bin/bash
mkdir -p /var/run/sshd
chmod 755 /var/run/sshd
...
EOFSSH
        ...
        if [ -f /etc/rc.local ]; then
            if ! grep -q "start-ssh" /etc/rc.local; then
                sed -i '/^exit 0/i /usr/local/bin/start-ssh.sh' /etc/rc.local
            fi
        else
            echo '#!/bin/bash' > /etc/rc.local
            echo '/usr/local/bin/start-ssh.sh' >> /etc/rc.local
            echo 'exit 0' >> /etc/rc.local
            chmod +x /etc/rc.local
        fi
        """
        run_docker_command(container_id, ["bash", "-c", ssh_startup_script])
...
def create_vps():
    ...
    if request.method == 'POST':
        try:
            ...
            setup_success, _ = setup_container(container.id, memory, vps_id, ssh_port, root_password, watermark, welcome, disk)
            ...
            time.sleep(3)  # Give SSH time to start
            verify_ssh_cmd = f"""
            ...
            if netstat -tlnp 2>/dev/null | grep -q ':22 ' || ss -tlnp 2>/dev/null | grep -q ':22 '; then
                ...
                echo "WARNING: SSH may not be listening. Attempting to start..."
                /usr/local/bin/start-ssh.sh || /usr/sbin/sshd -D &
                sleep 2
                exit 0
            fi
            """
            run_docker_command(container.id, ["bash", "-c", verify_ssh_cmd])
            logger.info(f"VPS {vps_id} created with SSH on port {ssh_port}")
    ...
    if request.method == 'POST':
        try:
            ...
            if recreate:
                ...
                watermark = db.get_setting('watermark', WATERMARK)
                welcome = db.get_setting('welcome_message', WELCOME_MESSAGE)
                setup_success, _ = setup_container(new_container.id, new_memory, vps_id, vps['port'], vps['root_password'], watermark, welcome, new_disk)
                if not setup_success:
                    new_container.stop()
...
def clone_vps(vps_id):
    ...
    try:
        ...
        watermark = db.get_setting('watermark', WATERMARK)
        welcome = db.get_setting('welcome_message', WELCOME_MESSAGE)
        setup_success, _ = setup_container(new_container.id, vps['memory'], new_vps_id, new_ssh_port, new_root_password, watermark, welcome, vps['disk'])
        if not setup_success:
            new_container.stop()
            new_container.remove()
            raise Exception('Setup failed')
...
def add_vps_port(vps_id):
    ...
    try:
        ...
        watermark = db.get_setting('watermark', WATERMARK)
        welcome = db.get_setting('welcome_message', WELCOME_MESSAGE)
        setup_success, _ = setup_container(new_container.id, vps['memory'], vps_id, vps['port'], vps['root_password'], watermark, welcome, vps['disk'])
        if not setup_success:
            new_container.stop()
            new_container.remove()
            return jsonify({'error': 'Setup failed'}), 500
